        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-[#1f2b1b] rounded-xl p-6 shadow-sm border border-gray-100 dark:border-white/5">
                    <h3 class="font-heading font-bold text-lg text-[#1e3f1b] dark:text-white mb-6">Order Items</h3>
                    <div class="space-y-4">
                        @foreach($pesanan->items as $item)
                        <div class="flex gap-4 items-start p-4 bg-gray-50 dark:bg-white/5 rounded-xl">
                            ...
                            <div class="flex-1">
                                <div class="flex justify-between items-start gap-4">
                                    <div>
                                        <h4 class="font-heading font-bold text-base text-[#1e3f1b] dark:text-white mb-1">
                                            {{ $item->produk->nama_produk }}
                                        </h4>
                                        <p class="text-sm text-gray-500 dark:text-gray-400 font-sans">{{ Str::limit($item->produk->deskripsi, 60) }}</p>
                                        <div class="text-xs text-[#1e3f1b] dark:text-gray-300 font-medium mt-1">Farmer: {{ $item->produk->petani->nama_lengkap ?? 'N/A' }}</div>
                                    </div>
                ...
                <div class="bg-white dark:bg-[#1f2b1b] rounded-xl p-6 shadow-sm border border-gray-100 dark:border-white/5">
                    ...
                    <div class="space-y-6">
                        ...
                        @endphp
                        <div class="p-4 bg-gray-50 dark:bg-white/5 rounded-xl {{ !$loop->last ? 'mb-4' : '' }}">
                            <div class="flex gap-4 items-start mb-4">
                                ...
                                <div class="flex-1">
                                    <h4 class="font-heading font-bold text-base text-[#1e3f1b] dark:text-white mb-1">{{ $item->produk->nama_produk }}</h4>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 font-sans">{{ Str::limit($item->produk->deskripsi, 60) }}</p>
                                </div>
                            </div>
                            ...
                            <div class="bg-white dark:bg-white/10 rounded-lg p-4">
                                <div class="flex items-center gap-1 mb-2">
                                    @for($i = 1; $i <= 5; $i++)
                                        <span class="material-symbols-outlined text-lg {{ $i <= $existingReview->rating ? 'text-yellow-400' : 'text-gray-300' }}">star</span>
                                        @endfor
                                        <span class="text-sm text-gray-500 ml-2">{{ $existingReview->tgl_dibuat->format('d M Y') }}</span>
                                </div>
                                @if($existingReview->komentar)
                                <p class="text-sm text-gray-600 dark:text-gray-300">{{ $existingReview->komentar }}</p>
                                @endif
                            </div>
                            @else
                            <!-- Review Form -->
                            <form action="{{ route('ulasan.store') }}" method="POST" class="bg-white dark:bg-white/10 rounded-lg p-4">
                                @csrf
                                <input type="hidden" name="id_item_pesanan" value="{{ $item->id_item }}">
                                <input type="hidden" name="id_produk" value="{{ $item->id_produk }}">

                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-[#1e3f1b] dark:text-white mb-2">Rating</label>
                                    <div class="flex items-center gap-1 star-rating">
                                        @for($i = 1; $i <= 5; $i++)
                                            <label class="cursor-pointer">
                                            <input type="radio" name="rating" value="{{ $i }}" class="hidden" required>
                                            <span class="material-symbols-outlined text-2xl text-gray-300 star-icon hover:text-yellow-400 transition-colors">star</span>
                                            </label>
                                            @endfor
                                    </div>
                                </div>
                                ...
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-[#1e3f1b] dark:text-white mb-2">Comment (Optional)</label>
                                    <textarea name="komentar" rows="2" placeholder="Share your experience..."
                                        class="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-[#1f2b1b] text-sm resize-none focus:ring-1 focus:ring-[#53be20] focus:border-[#53be20]"></textarea>
                                </div>

                                <button type="submit" class="px-4 py-2 bg-[#53be20] text-white rounded-lg text-sm font-semibold hover:bg-[#45a01b] transition-colors">
                                    Submit Review
                                </button>
                            </form>
                            @endif
            ...
            <!-- Right Column: Timeline, Shipping, Payment -->
