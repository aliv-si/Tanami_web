...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <form action="{{ route('admin.pesan-kontak.destroy', $pesan->id_pesan) }}" method="POST" onsubmit="return confirm('Hapus pesan ini?')">
            @csrf
            @method('DELETE')
        ...
        <div class="p-8 border-b border-gray-50 bg-gray-50/30">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="flex flex-col gap-1">
                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Pengirim</span>
                ...
                <div class="flex flex-col gap-1 md:text-right">
                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Waktu Terkirim</span>
        ...
        <div class="p-8">
            <div class="flex flex-col gap-6">
                <div class="flex flex-col gap-2">
                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Subjek</span>
                    <h2 class="text-xl font-bold text-gray-900 font-heading">{{ $pesan->subjek }}</h2>
                </div>

                <div class="flex flex-col gap-2">
                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Isi Pesan</span>
                ...
                <div class="mt-4 pt-8 border-t border-gray-100 flex flex-col gap-4">
                    <h3 class="text-sm font-bold text-gray-900 font-heading">Tindak Lanjut</h3>
                    <div class="flex items-center gap-4">
                        <a href="mailto:{{ $pesan->email }}?subject=Re: {{ $pesan->subjek }}"
                            ...
                            Balas via Email ({{ $pesan->email }})
                        </a>
                    </div>
                    <p class="text-xs text-gray-400 text-center italic">
                        *Mengklik tombol di atas akan membuka aplikasi email (Outlook, Gmail, dll) di perangkat Anda.
                    </p>
                </div>
            </div>
        </div>
