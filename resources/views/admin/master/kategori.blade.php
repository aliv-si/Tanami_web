...
<div class="max-w-[1400px] mx-auto space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/40 transition-colors group">
                        ...
                        <td class="px-6 py-6">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.kategori.destroy', $kategori->id_kategori) }}" method="POST" onsubmit="return confirm('Delete this category?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-8 text-center text-gray-400">No Data Found</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="px-6 py-4 bg-gray-50/30 border-t border-gray-50 flex items-center justify-between">
            <p class="text-xs text-gray-500 font-medium">Total <span class="text-tanami-dark font-bold">{{ count($kategoriList ?? []) }}</span> Category</p>
        </div>
    </div>
</div>
...
<dialog id="modalTambah" class="rounded-2xl p-0 backdrop:bg-black/50 w-full max-w-md">
    <form action="{{ route('admin.kategori.store') }}" method="POST" class="p-6">
        @csrf
        <h3 class="text-lg font-heading font-bold text-tanami-dark mb-4">Add Category</h3>
        <div class="space-y-4">
            <div>
...
<dialog id="modalEdit" class="rounded-2xl p-0 backdrop:bg-black/50 w-full max-w-md">
    <form id="formEdit" method="POST" class="p-6">
        @csrf
        @method('PUT')
        <h3 class="text-lg font-heading font-bold text-tanami-dark mb-4">Edit Category</h3>
        <div class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Category Name</label>
