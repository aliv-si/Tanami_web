...
<header class="bg-white border-b border-gray-200 sticky top-0 z-40">
    <div class="flex items-center justify-between px-8 py-4">
        <h1 class="text-2xl font-bold font-heading text-text-dark">Incoming Orders</h1>
        <div class="flex items-center gap-6">
            <form action="{{ route('petani.pesanan') }}" method="GET" class="relative hidden lg:block">
                <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">search</span>
                <input name="q" value="{{ $currentSearch ?? '' }}" class="pl-10 pr-4 py-2 bg-gray-50 border-gray-200 rounded-lg text-sm focus:ring-primary focus:border-primary w-80" placeholder="Search Order ID or buyer name..." type="text" />
            </form>
        </div>
    </div>
</header>
    ...
    <div class="space-y-4">
            ...
            <div class="p-6">
                    ...
                    <div class="flex items-center gap-4">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                ...
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">Pending Payment</span>
                                @break
                                @case('menunggu_verifikasi')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">Awaiting Verification</span>
                                @break
                                @case('dibayar')
                                ...
                                @break
                                @case('selesai')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#53be20]/10 text-[#53be20]">Completed</span>
                                @break
                                @case('dibatalkan')
                            ...
                            <p class="text-sm text-gray-500">{{ $order->tgl_dibuat->format('d M Y, H:i') }} • {{ $order->pembeli->nama_lengkap ?? 'Buyer' }}</p>
                        </div>
                    </div>
                ...
                <div class="flex items-center gap-3 py-3 border-t border-gray-100">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <img src="{{ asset('storage/' . $item->produk->foto) }}" alt="{{ $item->produk->nama_produk ?? '' }}" class="w-10 h-10 rounded-lg object-cover border border-gray-100" />
                        @else
                        ...
                        @endif
                        <div>
                            <p class="text-sm font-medium text-text-dark">{{ $item->produk->nama_produk ?? 'Product' }}</p>
                            <p class="text-xs text-gray-400">{{ $item->jumlah }} x Rp {{ number_format($item->harga_snapshot, 0, ',', '.') }}</p>
                        </div>
                    </div>
                    @endforeach
                    @if($order->items->count() > 3)
                    <span class="text-xs text-gray-400">+{{ $order->items->count() - 3 }} others</span>
                    @endif
                </div>
        ...
        <div id="kirim-modal-{{ $order->id_pesanan }}" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-xl max-w-md w-full p-6">
                <h3 class="text-lg font-bold font-heading mb-4">Input Shipping Receipt</h3>
                <form action="{{ route('petani.pesanan.kirim', $order->id_pesanan) }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Receipt Number <span class="text-red-500">*</span></label>
                        <input type="text" name="no_resi" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-primary focus:border-primary" placeholder="Enter shipping receipt number" />
                    </div>
